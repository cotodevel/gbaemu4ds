...
//loader quantities: 
char* readabelnameversionsschar[1] = {"fat:/gbaemu4ds/gbaemu4ds.nds"};			//{"0","1"...}
char* pathversionschar[1] = {"fat:/gbaemu4ds/gbaemu4ds.nds"};		//{"0","1"...}
//set offset order
u8 inputtoVersion[1] = 
{
	0	//just 1 so always at index 0
};

#define nummerVersions 1
...
//---------------------------------------------------------------------------------
	...
	*fake_heap_end = 0;
	...
	//plain parent homebrew (default-no args)
	if(argc != 2)
	{
		...
		//if(arcvsave != (char*)0) goto dataluncher;	//no
		...
		while(1)
		{
			...
			if (pressed&KEY_A)
			{
				...
				break;	//exit to ndsloadfile
			}
			if (pressed&KEY_DOWN && ausgewauhlt < (nummerVersions-1)){ ausgewauhlt++;}
			if (pressed&KEY_UP && ausgewauhlt > 0) {ausgewauhlt--;}
		}
