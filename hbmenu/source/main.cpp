...
char* pathversionschar[18] =
	{
		(char*)"fat:/gbaemu4ds/gbaemu4dsemu.nds"}
	;
...
//---------------------------------------------------------------------------------
	...
	*fake_heap_end = 0;
	...
	iprintf("\x1b[2J");
	iprintf("gbaemu4DS: original ichfly 2013 - gbalive4DS: Coto 2015 \n");
	...
	while(1) 
	{
		...
		else
		{
			...
			fread((char*)&gbaheaderf, 1, sizeof(gbaHeader_t),gbafile);
			patch2_t* entries = (patch2_t*)malloc(sizeof(patch2_t)*patchheader.listentr);
			
			fclose(gbafile);
			
			u32 ourcrc = swiCRC16(0x0,(void*)&patchheader,sizeof(gbaHeader_t)) + swiCRC16(0xFFFF,(void*)&patchheader,sizeof(gbaHeader_t)) << 16;
			//checksettings
			u32 matching = 0;
