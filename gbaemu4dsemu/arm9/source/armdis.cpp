...
//#define debuggerReadMemory(addr)    READ32LE(((u8*)&map[(addr)>>24].address[(addr) & map[(addr)>>24].mask]))

//#define debuggerReadHalfWord(addr)  READ16LE(((u8*)&map[(addr)>>24].address[(addr) & map[(addr)>>24].mask]))

//#define debuggerReadByte(addr)  map[(addr)>>24].address[(addr) & map[(addr)>>24].mask]

//0 nds mode / 1 gba mode
u8 debuggerReadByte(u32 addr,u8 is_gbamode){
    if(is_gbamode == 1)
        return (CPUReadByte_stack(addr));
    return *(u8*)addr;
}

u16 debuggerReadHalfWord(u32 addr,u8 is_gbamode){
    if(is_gbamode == 1)
        return (CPUReadHalfWord_stack(addr));
    return *(u16*)addr;
}

u32 debuggerReadMemory(u32 addr,u8 is_gbamode){
    if(is_gbamode == 1)
        return (CPUReadMemory_stack(addr));
    return *(u32*)addr;
}
...
int disArm(u32 offset, char *dest, int flags,u8 is_gbamode){
  u32 opcode = debuggerReadMemory(offset,is_gbamode);
      ...
      case 'a':
        if ((opcode&0x076f0000)==0x004f0000){
          ...
          *dest++ = ']';
          dest = addStr(dest, " (=");
          *dest++ = '$';
          dest = addHex(dest ,32, debuggerReadMemory(adr,is_gbamode));
          *dest++=')';
        }
...
int disThumb(u32 offset, char *dest, int flags,u8 is_gbamode){
  u32 opcode = debuggerReadHalfWord(offset,is_gbamode);
