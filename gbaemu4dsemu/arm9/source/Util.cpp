...
/*
//usage: "new filename" "top: width" "top: height" "video buffer"
bool utilWritePNGFile(const char *fileName, int w, int h, u8 *pix)
{
  ...
  //deprecated:if(setjmp(png_ptr->jmpbuf)) {
    if(setjmp((int*)png_get_io_ptr(png_ptr))) {
		png_destroy_write_struct(&png_ptr,NULL);
		fclose(fp);
    return false;
  }
...
void patchit(int romSize2)
{
	...
	for(int i = 0;i < patchnum;i++)
	{
		...
		case 0:
			...
			if(offsetgba + sectorsize < romSize2)
			{
				int coo = ftell(patchf);
				fseek(patchf,offsetthisfile,SEEK_SET);
				fread(rom + offsetgba,1,sectorsize,patchf);
...
u8 *utilLoad(const char *file,u8 *data,int size,bool extram)
{
	u8 *image = data;
	FILE *f = fopen(file, "rb");
	...
	int read_result = 0;
	read_result = fread(rom, 1, toread, f);
    
    //set up header
    memcpy((u8*)&GBAEMU4DS_IPC->gbaheader_static,(u8*)rom,sizeof(gbaHeader_t));
...
//volatile __attribute__((section(".itcm")))
...
u32 STMW_myregs(u32 opcode, u32 temp, u32 address,u32 val,u32 num){
...
void UPDATE_REG(u16 address, u16 value){
	WRITE16LE(((u8*)&GBAEMU4DS_IPC->ioMem[address]),value);
}
...
//Set up GBAEMU4DS_IPC->IE address
void set_gba_ipc_regs(){
    IE_ADDRESS = (u32)&GBAEMU4DS_IPC->IE;
}
...
int savetype=0;
char gamecode[6]; 
memcpy((char*)gamecode,(u8*)&GBAEMU4DS_IPC->gbaheader_static.gamecode,6);
