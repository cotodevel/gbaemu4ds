//coto: todo: redirect read/writes to each gba bg mode, so rendering is done directly through the ppu engines 
...
/*
__attribute__((section(".dtcm")))
u16 GBAEMU4DS_IPC->IE       = 0x0000;

__attribute__((section(".dtcm")))
u16 GBAEMU4DS_IPC->IF       = 0x0000;

__attribute__((section(".dtcm")))
u16 GBAEMU4DS_IPC->IME      = 0x0000;
*/
...
u16 GBAEMU4DS_IPC->DM0SAD_H = 0x0000;
...
bool GBAEMU4DS_IPC->timer2On = false;

int GBAEMU4DS_IPC->timer2Ticks = 0;

int GBAEMU4DS_IPC->timer2Reload = 0;

int GBAEMU4DS_IPC->timer2ClockReload  = 0;

u16 GBAEMU4DS_IPC->timer3Value = 0;

bool GBAEMU4DS_IPC->timer3On = false;

int GBAEMU4DS_IPC->timer3Ticks = 0;

int GBAEMU4DS_IPC->timer3Reload = 0;

int GBAEMU4DS_IPC->timer3ClockReload  = 0;
...
int CPULoadRom(const char *szFile,bool extram)
{
  ...
  /*GBAEMU4DS_IPC->ioMem = (u8 *)calloc(1, 0x400);
  if(GBAEMU4DS_IPC->ioMem == NULL) {
    systemMessage(MSG_OUT_OF_MEMORY, N_("Failed to allocate memory for %s"),
                  "IO");
    //CPUCleanUp();
    return 0;
...
//coto: I know we can redirect to below DMA but its slower, and this is only for FIFO DMA since uses hardware NDS memory controller when possible
__attribute__((section(".itcm")))
__attribute__((aligned(4)))
        ...
        case(0x07000000):
        {
            ...
            switch(d&0x000000ff){
                ...
                case 0xa0:{
                    if(transfer32) {
                        s &= 0xFFFFFFFC;
                        u32 fifo_fetch = *(u32*)(s);
                        GBAEMU4DS_IPC->FIFO_A_L[0] = (fifo_fetch)&0xff;
                        GBAEMU4DS_IPC->FIFO_A_L[1] = (fifo_fetch>>8)&0xff;
                        GBAEMU4DS_IPC->FIFO_A_H[0] = (fifo_fetch>>16)&0xff;
                        GBAEMU4DS_IPC->FIFO_A_H[1] = (fifo_fetch>>24)&0xff;
                    }
                    else{
                        s &= 0xFFFFFFFE;
                        u16 fifo_fetch = *(u16*)(s);
                        GBAEMU4DS_IPC->FIFO_A_L[0] = (fifo_fetch)&0xff;
                        GBAEMU4DS_IPC->FIFO_A_L[1] = (fifo_fetch>>8)&0xff;
                    }   
                }
                ...
                case(0xa4):{
                    if(transfer32) {
                        s &= 0xFFFFFFFC;
                        u32 fifo_fetch = *(u32*)(s);
                        GBAEMU4DS_IPC->FIFO_B_L[0] = (fifo_fetch)&0xff;
                        GBAEMU4DS_IPC->FIFO_B_L[1] = (fifo_fetch>>8)&0xff;
                        GBAEMU4DS_IPC->FIFO_B_H[0] = (fifo_fetch>>16)&0xff;
                        GBAEMU4DS_IPC->FIFO_B_H[1] = (fifo_fetch>>24)&0xff;
                    }
                    else{
                        s &= 0xFFFFFFFE;
                        u16 fifo_fetch = *(u16*)(s);
        ...
        default:{
            ...
            switch(d&0x000000ff){
                ...
                case 0xa0:{
                    if(transfer32) {
                        s &= 0xFFFFFFFC;
                        u32 fifo_fetch = CPUReadMemory(s);
                        GBAEMU4DS_IPC->FIFO_A_L[0] = (fifo_fetch)&0xff;
                        GBAEMU4DS_IPC->FIFO_A_L[1] = (fifo_fetch>>8)&0xff;
                        GBAEMU4DS_IPC->FIFO_A_H[0] = (fifo_fetch>>16)&0xff;
                        GBAEMU4DS_IPC->FIFO_A_H[1] = (fifo_fetch>>24)&0xff;
                        ...
                        s &= 0xFFFFFFFE;
                        u16 fifo_fetch = CPUReadHalfWord(s);
                        GBAEMU4DS_IPC->FIFO_A_L[0] = (fifo_fetch)&0xff;
                        GBAEMU4DS_IPC->FIFO_A_L[1] = (fifo_fetch>>8)&0xff;
                    }   
                }
...
void CPUCheckDMA(int reason, int dmamask)
{
  ...
  if((GBAEMU4DS_IPC->DM1CNT_H & 0x8000) && (dmamask & 2)) {
    if(((GBAEMU4DS_IPC->DM1CNT_H >> 12) & 3) == reason) {
      ...
      //do DMA IRQ here
      GBAEMU4DS_IPC->DM1CNT_H |= (1<<14);
   
      cpuDmaHack = true;
      
      if(GBAEMU4DS_IPC->DM1CNT_H & 0x4000) {
        GBAEMU4DS_IPC->IF |= 0x0200;
        UPDATE_REG(0x202, GBAEMU4DS_IPC->IF);
        //cpuNextEvent = cpuTotalTicks;
      } 
  ...
  if((GBAEMU4DS_IPC->DM2CNT_H & 0x8000) && (dmamask & 4)) {
    if(((GBAEMU4DS_IPC->DM2CNT_H >> 12) & 3) == reason) {
      ...
      //do DMA IRQ here
      GBAEMU4DS_IPC->DM2CNT_H |= (1<<14);
      cpuDmaHack = true;
        
      if(GBAEMU4DS_IPC->DM2CNT_H & 0x4000) {
        GBAEMU4DS_IPC->IF |= 0x0400;
        UPDATE_REG(0x202, GBAEMU4DS_IPC->IF);
        //cpuNextEvent = cpuTotalTicks;
      }
...
//SOUND EVENT: 
  ...
  case 0xa6:
  {
        ...
        SendArm7Command(0xc0700100,address,value,0x0);
    ...
    case 0x88:{
        SendArm7Command(0xc0700104,value,0x8,0x0);
    }
    ...
    //If bit 12-13 == 3, get ARM7 to issue a DMA to FIFO transfer
    case 0xC6:
    {
    ...
    //If bit 12-13 == 3, get ARM7 to issue a DMA to FIFO transfer (doDMA)
    case 0xD2:
    {
 ...
 case 0x100:
  ...
  case 0x200:
	...
	REG_IE = GBAEMU4DS_IPC->IE | (REG_IE & 0xFFFF0000) | IRQ_HBLANK ; //NDS + GBA IRQs MUST be treated
	
	IEBACKUP = GBAEMU4DS_IPC->IE;  //Banked GBA IRQ GBAEMU4DS_IPC->IE treated right after arm9 irq handler
	
    break;
  case 0x202:
    //GBAEMU4DS_IPC->IF = value & 0x3FFF;
    //UPDATE_REG(0x204, GBAEMU4DS_IPC->IF);
...
void CPUReset()
{
  ...
  // clean io memory
  memset((u8*)GBAEMU4DS_IPC->ioMem, 0, 0x400);
  ...
  map[3].mask = 0x7FFF;
  map[4].address = (u8*)GBAEMU4DS_IPC->ioMem;
  map[4].mask = 0x3FF;
  ...
  case 4:
    ...
    if((address < 0x4000400) && ioReadable[address & 0x3fc]) {
      if(ioReadable[(address & 0x3fc) + 2])
        value = READ32LE(((u8 *)&GBAEMU4DS_IPC->ioMem[address & 0x3fC]));
      else
        value = READ16LE(((u8 *)&GBAEMU4DS_IPC->ioMem[address & 0x3fc]));
...
u16 CPUReadHalfWord(u32 address) //ichfly not inline is faster because it is smaler
{
  ...
  case 4:
    ...
    if(address > 0x40000FF && address < 0x4000111)
	{
		...
		if((address&0x2) == 0)
		{
			if(GBAEMU4DS_IPC->ioMem[address & 0x3fe] & 0x8000)
			{
    ...
    {
		value =  READ16LE(((u8 *)&GBAEMU4DS_IPC->ioMem[address & 0x3fe]));
    }
    else 
        goto unreadable;
...
iprintf("r8 %02x\n",address);
  ...
  case 4:
    ...
    if((address < 0x4000400) && ioReadable[address & 0x3ff])
      return GBAEMU4DS_IPC->ioMem[address & 0x3ff];
    else goto unreadable;
	...
	if(address < 0x4000400) {
		switch(address & 0x3FF) {
			...
			case 0x9f:
				...
				SendArm7Command(0xc0700100,(u32)address&0xFF, (u32)b,0x0);
			break;
			...
			default: // every other register
				...
				if(address & 1) {
					CPUUpdateRegister(lowerBits, (READ16LE((u8*)&GBAEMU4DS_IPC->ioMem[lowerBits]) & 0x00FF) | (b << 8));
				} 
				else 
				{
					CPUUpdateRegister(lowerBits, (READ16LE((u8*)&GBAEMU4DS_IPC->ioMem[lowerBits]) & 0xFF00) | b);
				}
		}
	}
	else 
